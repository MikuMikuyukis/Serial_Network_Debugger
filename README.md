...
通信设置保存在当前浏览器的本地存储中。刷新或重新打开页面后会恢复设置，但不会自动建立连接。不同浏览器或不同电脑的设置彼此独立。
...
测试会使用本机回环地址创建临时 TCP/UDP 端点，不访问公网，也不要求真实串口。项目迁移到 Node.js 后共有 27 项自动测试，涵盖编解码、配置校验、串口接收合并、TCP/UDP、HTTP 和 WebSocket 实时事件链路。
