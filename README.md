...
- 串口枚举、连接、收发，支持波特率、数据位、校验位和停止位配置
...
- 浏览器最多保留 5,000 条日志，并以 50 ms 为周期批量渲染
- FastAPI 自动生成的接口文档
...
串口是字节流，一次发送的数据可能被拆成多条 RX 日志，例如 6 字节回环可能按 1、1、4 字节到达。这不代表丢包，判断时应比较累计 RX 字节数和连续数据内容。选择 HEX 发送后，界面会自动启用 HEX 显示，避免 `00` 等不可见字节看起来像空内容。
