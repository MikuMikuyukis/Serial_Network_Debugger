...
- 发送预设的新增、编辑、删除、载入与一键发送，完整保存内容、格式、编码和行尾参数
...
测试会使用本机回环地址创建临时 TCP/UDP 端点，不访问公网，也不要求真实串口。项目共有 30 项自动测试，涵盖编解码、配置校验、串口接收合并、TCP/UDP、HTTP、WebSocket 实时事件链路和周期发送调度。
...
- 当前版本没有日志落盘、协议组帧和动态快速控件，这些属于后续功能。
